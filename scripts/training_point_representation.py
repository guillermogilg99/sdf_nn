...
def PC_POS_callback(PC_msg, POS_msg):
    #--Parameters--
    dist_between_iter = 2 # Meters between two training iterations
    ...
    points_per_ray = 10 # Points per LiDAR ray
    ...
    if (np.sqrt((x_pos-x_last)**2 + (y_pos-y_last)**2 + (z_pos-z_last)**2) > dist_between_iter or 360/np.pi*np.arccos(acos_arg) > ang_between_iter):
        ...
        num_samples_wallpoints = pointcount // 10 #Wall points to be considered (from 0 to pointcount)
        ...
        num_samp_ray = pointcount // 10 #LiDAR rays to be considered
